...
async def periodic_task():
    while True:
        ...
        # 10 hours * 60 minutes/hour * 60 seconds/minute
...
def get_w3_and_contract(chain: str):
    if chain in app.config['CONTRACTS']:
        ...
        contract = w3.eth.contract(
            address=config['ADDRESS'], abi=load_contract_abi())
        return w3, contract
    else:
...
async def setup_distributed_scheme(party_number, pool) -> DistributedPaillier:
    with open(f"store/{party_number}.pkl", 'rb') as f:
        data = pickle.load(f)
    paillier_public_key = PaillierPublicKey.deserialize(
        data['paillier']['pubkey'])
    paillier_shared_key = PaillierSharedKey.deserialize(
        data['paillier']['seckey'])
...
async def sum_encrypted_values(orders):
    ...
    for order in orders:
        ciphertext = PaillierCiphertext(
            int(order['encrypted_order_value']), distributed_schemes[0])
...
async def binary_search_and_partial_decrypt(cumulative_sums, target_value, matched_orders, orders):
    ...
    for order in partial_matched_orders[:-1]:
        encrypted_order_value = PaillierCiphertext(
            int(order['encrypted_order_value']), app.distribute_scema[0])
    ...
    if remaining_encrypted_value != app.distribute_scema[0].encrypt(0):
        ...
        target_value_dec = await decrypt_all_orders([target_value])

        decrypted_order_value = await decrypt_all_orders([temp])
        print("value", decrypted_order_value)
        matched_orders.append({
            ...
            'encrypted_order_value': abs(int(target_value_dec[0])) 
        })


        # Update the remaining value and add to remaining_orders
...
async def execute_orders_internal():
    print("welcome")
    orders = load_orders()
    eth_to_usdc_orders = [order for order in orders if order['sellToken'] == app.config['TOKEN_ADDRESSES']
                          [order['chain']]['ETH'] and order['buyToken'] == app.config['TOKEN_ADDRESSES'][order['chain']]['USDC']]
    usdc_to_eth_orders = [order for order in orders if order['sellToken'] == app.config['TOKEN_ADDRESSES']
                          [order['chain']]['USDC'] and order['buyToken'] == app.config['TOKEN_ADDRESSES'][order['chain']]['ETH']]
    ...
    # decrypted_order_values = [await decrypt_all_orders([ciphertext]) for ciphertext in cumulative_sums]

    # # Print each decrypted value
    # for decrypted_value in decrypted_order_values:
    #     print("Decrypted cumulative sum:", decrypted_value)
    matched_orders = []
...
async def execute_matched_orders(request: List[dict]):
    try:
        ...
        for order in request:
            ...
            # Replace "ETH_ADDRESS" with actual ETH address
            if order['buyToken'] == w3.to_checksum_address("0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE"):
                # Buying ETH with USDC
                eth_amount = buy_eth_with_usdc(
                    order['encrypted_order_value'], eth_price)
                fetched_amount = int(eth_amount)
            # Replace "ETH_ADDRESS" with actual ETH address
            elif order['sellToken'] == w3.to_checksum_address("0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE"):
                # Selling ETH for USDC
                usdc_amount = sell_eth_for_usdc(
                    order['encrypted_order_value'], eth_price)
